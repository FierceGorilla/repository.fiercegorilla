# -*- coding: utf-8 -*-
# v1.06 With edits by Tomb Raider & Origin
import urllib
import urllib2
import datetime
import shutil
...
def SKindex():
    addon_log("SKindex")
    addDir('[B][COLOR blue]FIND US ON TWITTER @dogsbollocks_ZT[/B][/COLOR]','[B][COLORblue]FIND US ON TWITTER @dogsbollocks_ZT[/B][/COLOR]',4,'https://cdn1.iconfinder.com/data/icons/logotypes/32/square-twitter-256.png' ,  FANART,'','','','')	
    addDir('[B][I][COLORred]"MOVIE & CATCH UP TV SEARCH"[/B][/I][/COLOR]','[B][I][COLORred]"MOVIE & CATCH UP TV SEARCH"[/B][/I][/COLOR]',41,'https://freeiconshop.com/wp-content/uploads/edd/search-flat.png' ,  FANART,'','','','')
	...
	if Search_name == '':
		...
		for name2,url2,image2,fanart2 in match2:
			...
			if 'sublink' in url2:
				...
				if Search_title in name2.lower().replace(' ',''):
					addDir(name2,url2,1,image2,fanart2,'','','','')	
		HTML43 = Open_Url('http://supremacy.org.uk/tombraider/dogsbollocks/marchtv.txt')
		match2 = re.compile('<title>(.+?)</title>.+?<link>(.+?)</link>.+?<thumbnail>(.+?)</thumbnail>.+?<fanart>(.+?)/fanart>',re.DOTALL).findall(HTML43)
		for name2,url2,image2,fanart2 in match2:
			if fanart2 == '<':
				fanart2 = FANART
			else:
			...
			if 'sublink' in url2:
				...
				if Search_title in name2.lower().replace(' ',''):
					addDir(name2,url2,1,image2,fanart2,'','','','')	
		HTML44 = Open_Url('http://supremacy.org.uk/tombraider/dogsbollocks/apriltv.txt')
		match2 = re.compile('<title>(.+?)</title>.+?<link>(.+?)</link>.+?<thumbnail>(.+?)</thumbnail>.+?<fanart>(.+?)/fanart>',re.DOTALL).findall(HTML44)
		for name2,url2,image2,fanart2 in match2:
			if fanart2 == '<':
				fanart2 = FANART
			else:
			...
			if 'sublink' in url2:
				...
				if Search_title in name2.lower().replace(' ',''):
					addDir(name2,url2,1,image2,fanart2,'','','','')
		HTML45 = Open_Url('http://supremacy.org.uk/tombraider/dogsbollocks/maytv.txt')
		match2 = re.compile('<title>(.+?)</title>.+?<link>(.+?)</link>.+?<thumbnail>(.+?)</thumbnail>.+?<fanart>(.+?)/fanart>',re.DOTALL).findall(HTML45)
		for name2,url2,image2,fanart2 in match2:
			if fanart2 == '<':
				fanart2 = FANART
			else:
...
#                print channel
                ...
                try:
                    if name == '[B][I][COLORgold]YOU ARE IN THE MOVIE ZONE[/B][/I][/COLOR]':
                        ...
                        addDir('[B][I][COLORred]"MOVIE & CATCH UP TV SEARCH"[/B][/I][/COLOR]','[B][I][COLORred]"MOVIE & CATCH UP TV SEARCH"[/B][/I][/COLOR]',41,'http://spiritualunitecdn-8df5.kxcdn.com/articles/wp-content/uploads/2016/06/pyramid-eye-third-eye1.jpg' ,  FANART,'','','','')
                    elif linkedUrl=='':
                        addDir(name.encode('utf-8', 'ignore'),url.encode('utf-8'),2,thumbnail,fanArt,desc,genre,date,credits,True)
                    else:
